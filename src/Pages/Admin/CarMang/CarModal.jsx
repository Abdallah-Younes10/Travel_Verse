...
function CarModal({ isOpen, onClose, initialData }) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (Array.isArray(initialData.images)) {
        ...
        setPreviewImages(initialData.images.map(img => `http://localhost:8000${img.image_path}`));
      } else {
        setOldImages([]);
        setPreviewImages([]);
      }
  ...
  const addCar = useMutation({
    mutationFn: async (formData) => {
      return await axios.post('http://localhost:8000/api/cars', formData, {
  ...
  const updateCar = useMutation({
    mutationFn: async ({ id, formData }) => {
      return await axios.post(`http://localhost:8000/api/cars/${id}?_method=PUT`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
    },
    onSuccess: () => {
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '10px' }}>
            ...
            {oldImages.map((img, i) => (
              <div key={`old-${i}`} style={{ position: 'relative' }}>
                <img src={`http://localhost:8000${img.image_path}`} alt="old" style={{ width: '100px', height: '80px' }} />
