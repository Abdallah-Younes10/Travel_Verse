...
function CarManagement() {
  ...
  const fetchCars = async ({ queryKey }) => {
    ...
    const response = await axios.get(`http://localhost:8000/api/cars?page=${page}&search=${searchTerm}`);
  ...
  const deleteCar = useMutation({
    mutationFn: async (id) => await axios.delete(`http://localhost:8000/api/cars/${id}`),
    onSuccess: () => {
