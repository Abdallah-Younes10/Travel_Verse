...
import axios from '../../Auth/axios'; // ✅ استخدم النسخة التي فيها التوكن
...
function TripManagement() {
  ...
  const addTripMutation = useMutation({
    mutationFn: async (tripData) => {
      ...
      const payload = {
        ...rest,
        guide_id: has_guide ? tripData.guide_id || 1 : null,
      };
      ...
      if (images.length) {
        ...
        await axios.post(`/trips/${res.data.id}/images`, formData);
      }
      if (tripData.image_urls?.length) {
  await Promise.all(
    tripData.image_urls.map(url =>
      axios.post(`/trips/${res.data.id}/images`, {
  ...
  const editTripMutation = useMutation({
    mutationFn: async (tripData) => {
      ...
      if (images.length && typeof images[0] !== 'string') {
        const formData = new FormData();
        images.forEach(img => formData.append('images[]', img));
        await axios.post(`/trips/${id}/images`, formData);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['trips'] });
