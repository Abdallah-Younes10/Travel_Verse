...
const fetchAllHotels = async () => {
    ...
    do {
        const res = await axios.get(
            `http://localhost:8000/api/hotels?page=${page}`,
        );
...
const Hotel = () => {
            ...
            if (values.length > 0) {
                result = result.filter((hotel) => {
                    ...
                    if (key === "style") {
                        ...
                        return values.some(
                            (v) =>
                                hotel.style?.toLowerCase() === v.toLowerCase(),
                        );
                    }
                    if (key === "hotelClass") {
    ...
    const renderFilters = () => (
        <Box sx={{ width: 250, p: 2 }} role="presentation">
            <h5 className="mb-3">{t("filters")}</h5>
    ...
    return (
        <div>
            ...
            <Container>
                <Row className={`${style.head} mb-3 align-items-center`}>
                    <Col xs={12} md={6}>
