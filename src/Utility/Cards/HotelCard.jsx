...
const HotelCard = ({ data, type }) => {
    ...
    const handleAddToWishlist = async () => {
        try {
            await axios.post("http://localhost:8000/api/favorites", {
                ...
                favoritable_type: `App\\Models\\${detectedType.charAt(0).toUpperCase() + detectedType.slice(1)}`,
            });
    ...
    return (
        <MDBRow className="justify-content-center mb-3">
            <MDBCol md="12" xl="10">
                <MDBCard className="shadow-0 rounded-xl! bg-transparent h-full shadow-lg hover:shadow-xl transition-shadow duration-300 ">
                        ...
                        <MDBRow>
                            <MDBCol md="12" lg="5" className="mb-4 mb-lg-0" style={{ position: 'relative' }}>
                                ...
                                <div style={{ position: 'absolute', top: 8, right: 8, zIndex: 10 }}>
                                  <WatchlistButton type={detectedType} id={data.id} />
                                </div>
                            </MDBCol>
